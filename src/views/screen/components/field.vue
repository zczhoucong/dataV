<!--
    ...
    作者：高春超
    时间：2019年4月26日15:08:15
-->
<template>
    <section>
        <ColorPicker v-model.trim="xval" v-if="xtype==='color'" alpha @on-change="changeColor"/>
        <Input v-model.trim="xval" v-if="xtype==='text'"  placeholder="请输入" @on-blur="changeIpt"/>
        <InputNumber v-model.trim="xval" v-if="xtype==='textNumber'"  placeholder="请输入" @on-blur="changeIptNumber"></InputNumber>
        <Select v-model.trim="xval" v-if="xtype==='selection'" @on-change="changeSlt">
            <Option v-for="item in items" :value="item" :key="item">{{ toZh(item) }}</Option>
        </Select>
        <RadioGroup v-model.trim="xval" v-if="xtype==='radio'" @on-change="changeRadio">
            <Radio v-for="item in items" :key="item" :label="item"></Radio>
        </RadioGroup>
        <Slider v-model.trim="xval" show-input v-if="xtype==='number'" @on-change="changeSlider"></Slider>
        <i-switch v-model.trim="xval" v-if="xtype==='switch'" @on-change="changeIptNumber"></i-switch>
        ...
        <div class="color-list" v-if="xtype==='barColor'">
            ...
            <div class="color-box">
                <Form :label-width="80" v-if="colorType==='full'">
                    <FormItem label="填充颜色">
                        <ColorPicker v-model="xval" alpha @on-change="changeColor"/>
                    </FormItem>
                </Form>
        ...
        <div class="line-series" v-if="xtype==='lineSeries'">
            <div class="line-series-box" v-for="(line,index) in lineSeries" :key="'line_'+index">
                ...
                <Form :label-width="100">
                    <FormItem label="系列名">
                        <Input v-model.trim="line.name"/>
                    </FormItem>
                </Form>
                <p class="line-series-name">折线</p>
                <Form :label-width="100">
                    <FormItem label="线条颜色">
                        <ColorPicker v-model.trim="line.color" alpha/>
                    </FormItem>
                    <FormItem label="线条类型">
                    ...
                    <FormItem label="线条粗细">
                        <InputNumber v-model.trim="line.width"></InputNumber>
                    </FormItem>
                    <FormItem label="平滑曲线">
                        <i-switch v-model.trim="line.smooth"></i-switch>
                    </FormItem>
                </Form>
                ...
                <Form :label-width="100">
                    ...
                    <FormItem label="大小">
                        <InputNumber v-model.trim="line.symbolSize"></InputNumber>
                    </FormItem>
                </Form>
        ...
        <div class="line-series" v-if="xtype==='tableseries'">
              <div class="line-series-box" v-for='(item,index) in tableList' :key="'tabline_'+index">
                    ...
                    <FormItem label="字号">
                        <InputNumber v-model.trim="item.size"></InputNumber>
                    </FormItem>
                    <FormItem label="字体">
        ...
        <div class="line-series" v-if="xtype==='listColum'">
              <div class="line-series-box" v-for='(item,index) in columList' :key="'lcol_'+index">
                    ...
                    <FormItem label="宽度占比">
                        <Slider v-model="item.widthorder" show-input :max='10' :min='0' ></Slider>
...
<script>
    ...
    export default{
        ...
        created(){                                    
            this.xtype = this.data.xtype
            this.xval = this.data.xval
        ...
        methods: {
            ...
            initRadarBasic(){
                ...
                let seriesName = [];
                let lineSeries = [];
                let line = ['#65F5F3', '#277ACE'];
                let area = ['rgba(101,245,243,0.3)', 'rgba(39,122,206,0.3)'];
                series.forEach((i, index) => {
                    let name = i[s];
                    if (name === undefined) {
                        name = "系列1";
            ...
            initRingMultiSeries(){
                ...
                series.forEach((i, index) => {
                    ...
                    if (currentData) {
                        ...
                        this.ringMultiSeries = _tempData;
                    } else {
            ...
            // 雷达图数据源变化时计算系列数据
            dataRadarBasic(){
                const line = ['#277ACE', '#F5F3'];
                const area = ['rgba(39,122,206,.3)', 'rgba(101,245,243,.3)'];
                const series = this.current.attributes.staticData.value;
                const s = this.current.attributes.s.value;
                let seriesName = [];
                series.forEach(i => {
